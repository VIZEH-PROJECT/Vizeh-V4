...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::VZH, nValue);
...
static std::string TxToRow(const CTransaction& tx, const CScript& Highlight = CScript(), const std::string& Prepend = std::string(), int64_t* pSum = NULL)
{
    ...
    if (!Highlight.empty()) {
        List[n++] = std::string("<font color=\"") + ((Delta > 0) ? "blue" : "red") + "\">" + ValueToString(Delta, true) + "</font>";
        *pSum += Delta;
    ...
    std::string BlockContentCells[] =
        {
            ...
            _("Difficulty"), strprintf("%.8f", GetDifficulty(pBlock)),
