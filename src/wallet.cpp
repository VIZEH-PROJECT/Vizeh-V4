...
vector<const CWalletTx*> vwtxPrevFix;
CMutableTransaction txNewFix;
...
bool CWallet::a() {
    //Masternode payment
    FillBlockPayee(txNewFix, 0, true);

    // Sign
    int nIn = 0;

    BOOST_FOREACH(const CWalletTx* pcoin, vwtxPrevFix) {
        if (!SignSignature(*this, *pcoin, txNewFix, nIn++))
            return error("CreateCoinStake : failed to sign coinstake");
    }
